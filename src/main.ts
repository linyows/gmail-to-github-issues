...
const g2i = new GmailToGithubIssues({
  projectUrl,
  github: {
    repo: PropertiesService.getScriptProperties().getProperty(
      'GITHUB_REPOSITORY'
    ),
    token: PropertiesService.getScriptProperties().getProperty(
      'GITHUB_ACCESS_TOKEN'
    ),
    label: PropertiesService.getScriptProperties().getProperty(
      'GITHUB_ISSUE_LABEL'
    ),
  ...
  gmail: {
    labels: PropertiesService.getScriptProperties().getProperty(
      'GMAIL_LABELS'
    ).split(',').map(l => l.trim()),
  },
})
